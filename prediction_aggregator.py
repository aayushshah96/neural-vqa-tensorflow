...
def find_same_first_answer(m1_predictions_path, m2_predictions_path, m3_predictions_path):
    ...
    if len(predictions_1) == len(predictions_2) == len(predictions_3):
        ...
        raise Exception('predictions have different lengths, M1: {0}, M2: {1}, M3: {2}'. format(len(predictions_1), len(predictions_2), len(predictions_3)))


if __name__ == '__main__':
    solution = find_same_first_answer('result.txt', 'result2.txt', 'result3.txt')
    with open('aggregated_results.json','w') as output_file:
